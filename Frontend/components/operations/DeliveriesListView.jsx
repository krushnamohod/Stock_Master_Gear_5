...
import { AlertCircle, Calendar, CheckCircle, Clock, Package, User, XCircle } from "lucide-react"

export function DeliveriesListView({ searchTerm, onOperationClick }) {
    const { operations, theme } = useStock()

    // Filter deliveries only
    const deliveries = operations.filter(op => op.type === 'DELIVERY')
        .filter(op =>
            op.reference.toLowerCase().includes(searchTerm.toLowerCase()) ||
            (op.contact && op.contact.toLowerCase().includes(searchTerm.toLowerCase())) ||
                ...
                <table className="w-full text-sm text-left">
                    <thead className={cn(
                        ...
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left font-medium">Delivery Person</th>
                    ...
                    <tbody className={cn("divide-y", theme === 'dark' ? "divide-slate-800" : "divide-slate-200")}>
                        {deliveries.map((delivery) => (
                            <tr
                                key={delivery.id}
                                onClick={() => onOperationClick(delivery)}
                                className={cn(
                            ...
                            >
                                ...
                                <td className={cn("px-6 py-4", theme === 'dark' ? "text-slate-400" : "text-slate-600")}>
                                    {delivery.productLines.length} item{delivery.productLines.length !== 1 ? 's' : ''}
                                </td>
