...
import { AlertCircle, Calendar, CheckCircle, Clock, MapPin, Settings, XCircle } from "lucide-react"

export function AdjustmentsListView({ searchTerm, onOperationClick }) {
        ...
        <div className={cn("rounded-lg border overflow-hidden", theme === 'dark' ? "border-slate-800" : "border-slate-200")}>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className={cn("divide-y", theme === 'dark' ? "divide-slate-800" : "divide-slate-200")}>
                        {adjustments.map((adjustment) => (
                            <tr
                                key={adjustment.id}
                                onClick={() => onOperationClick(adjustment)}
                                className={cn(
                            ...
                            >
                                ...
                                <td className={cn("px-6 py-4", theme === 'dark' ? "text-slate-400" : "text-slate-600")}>
                                    <div className="flex items-center gap-2">
                                        ...
                                        {adjustment.destLocation}
                                    </div>
                                </td>
                                <td className={cn("px-6 py-4", theme === 'dark' ? "text-slate-400" : "text-slate-600")}>
                                    {adjustment.productLines.length} item{adjustment.productLines.length !== 1 ? 's' : ''}
                                </td>
            ...
            {adjustments.length === 0 && (
                <div className="text-center py-12">
                    <Settings className={cn("h-12 w-12 mx-auto mb-4", theme === 'dark' ? 'text-slate-600' : 'text-slate-400')} />
